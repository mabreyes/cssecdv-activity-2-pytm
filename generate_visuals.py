...
def generate_dfd_png():
    ...
    try:
        ...
        cmd = [
            'python3', 'web_feedback_system_dfd.py', '--dfd'
        ]
...
def generate_sequence_png():
    ...
    if not plantuml_jar.exists():
        ...
        try:
            ...
            print("Please download plantuml.jar manually from http://plantuml.com/download")
            return False
    ...
    try:
        ...
        cmd = [
            'python3', 'web_feedback_system_dfd.py', '--seq'
        ]
        
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
        seq_content = result.stdout
        
        # Extract only the PlantUML content
        ...
        if plantuml_start >= 0:
            ...
            if plantuml_end >= 0:
                ...
                subprocess.run(plantuml_cmd, check=True)
...
def main():
    """Main function to generate visual diagrams."""
